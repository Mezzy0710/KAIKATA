...
✅ **Review fixes** (extension 2.1.0): an empty wants page ("There are no offers…", or no rows with the `idWantslist` filter present) is `isEmpty`, not a challenge; it is saved as a 0-offer capture (`Flow.emptyCapture`) that counts as loaded everywhere, and KAIKATA's impact text says "checked, nothing extra" (`checkedText`). The cart is read once: Cardmarket renders each seller block twice (desktop + hidden mobile layout); `content-script.js` keeps rendered sections/rows/links only (`getClientRects`), and pure `extension/cartforge-cart-rows.js` dedupes by article id, then by card/collector/condition/price/quantity while rows exceed "Contents N Articles". "Check all sellers" (cart page, click only) fetches page 1 of each unloaded seller's wants page via `CartforgeWantsParser.checkSellersFirstPage` (walker stop rules, 2–4 s apart); empty → 0-offer capture, offers → `cartforgeWantsCheckV1` ("H offers on P pages", sorted first, not saved as offers). Cart panel: bottom-left, ≤320 px, expanded state in `cartforgePanelExpandedV1`, Copy / website under "More". Wants panel: max-height 100vh − 32 px, sticky progress header
...
- ✅ Cart rows (`cart-rows.mjs`, fixture `cart-duplicated-layouts.html`): desktop + mobile layouts → 4 sellers, 16 articles = sum of Contents, wants links deduped, both viewports and the no-visibility fallback
- ✅ Empty wants page (`wants-page-empty.html`): `isEmpty`, walker stops after page 1, "Check all sellers" page-1 checks + stop rules (`wants-parser.mjs`); 0-offer captures loaded / skipped by next seller / checklist order after checks (`wants-flow.mjs`)
...
│   ├── cartforge-cart-rows.js      # Pure: rendered-row pick + dedupe (cart has desktop + mobile copies)
...
Last Updated: September 2026 (review fixes, 2.1.0)
Branch: `main`
